...
DeleteAccount::preclaim(PreclaimContext const& ctx)
{
    ...
    // account re-creation. 
    //
    // Without this restriction, duplicate NFTokenIDs can be reproduced when authorized 
    // minting is involved. Because when the minter mints a NFToken, the issuer's 
    // sequence does not change. So when the issuer re-creates their account and 
    // mints a NFToken, it is possible that the NFTokenSequence of this NFToken is 
    // the same as the one that the authorized minter minted in a previous ledger.  
