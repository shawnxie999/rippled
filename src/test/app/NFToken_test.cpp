...
class NFToken_test : public beast::unit_test::suite
{
    ...
    {
        // `nftoken_ids` is added in the `tx` response for NFTokenMint,
        // NFTokenAcceptOffer and NFTokenCancelOffer `offer_id` is added in the
        // `tx` response for NFTokenCreateOffer The value of these fields is
        // dependent on the NFTokenID/OfferID changed in its corresponding
        // transaction. We want to validate each transaction to make sure the
        // synethic fields hold the right values.
        ...
        // Check `nftoken_ids and `offer_id` in tx meta when for all NFT transactions
        {
            // Alice mints 2 NFTs
        ...
        // Check `nftoken_ids` in brokered mode
        {
            ...
            uint256 const offerBobToBroker =
                keylet::nftoffer(bob, env.seq(bob)).key;
            env(token::createOffer(bob, nftId, drops(1)),
                token::owner(alice));
            env.close();
